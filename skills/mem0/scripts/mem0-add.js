...
async function addMemory() {
  ...
  try {
    ...
    // Local fast-path: skip LLM fact extraction/inference to avoid long CPU-bound timeouts.
    // This stores the provided text/messages directly into vector memory.
    const addConfig = { userId, infer: false };

    let result;
    if (messages) {
      result = await memory.add(messages, addConfig);
    } else {
      result = await memory.add(text, addConfig);
    }
